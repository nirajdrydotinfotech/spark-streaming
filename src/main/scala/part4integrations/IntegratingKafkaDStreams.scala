...
object IntegratingKafkaDStreams {
  ...
  val spark=SparkSession.builder()
    .appName("Integrating kafka in DStreams")
    .master("local[2]")
    .getOrCreate()
  ...
  val kafkaParams:Map[String,Object]=Map(
    ...
    "key.serializer" -> classOf[StringSerializer], //send data to the kafka
  ...
  def readFromKafka()={
    ...
    val kafkaDStream= KafkaUtils.createDirectStream(
      ...
      /*
      Distributed the partitions evenly across the spark cluster
      Alternatives-
      ...
      /*
      Alternative
      -SubscribePattern allows subscribing to topics matching pattern
      -Assign -advanced ;allows specifying offsets and partition per cluster
       */
    )
  ...
  def writeToKafka()={
    ...
    processedData.foreachRDD{rdd=>
      rdd.foreachPartition{partition=>
        //inside this lambda ,the code is run by single executor
